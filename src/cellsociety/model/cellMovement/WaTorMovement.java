 ...
 * the movement of the Sharks and Fish depending on the neighbors they have. If the rules class
 * returns that a cell needs to move to a region of different neighbors, this class selects a
 * neighboring cell for the cell to move to, and immediately reflects this in the current state of
 * the cell. It utilizes consumers to deal with differing cell types and the actions that they
 * should take. It also calls the Developer class to keep track of reproduction and energy states
 * for each cell, and make changes that reflect those states when needed.
 */
public class WaTorMovement implements CellSocietyMovement {
  ...
  /**
   * This is the constructor for the WaTorMovement class. It sets up an instance of the Developer
   * class that will be used to deal with cell reproduction and energy. It also creates an ArrayList
   * that keeps track of cells that have already been changed (and should not be changed again until
   * the view has reflected these changes).
   */
  ...
  /**
   * This method sets up initial parameters (after being called by WaTorModel) to be used throughout
   * the class
   *
   ...
   * @param statesBundle: the resource bundle that holds all of the relevant states for the WaTor
   *                      Game.
   * @param parameters:   the map of all the relevant parameters for the WaTor game as read from the
   *                      .sim file.
   */
  ...
  /**
   * This method returns the list of already changed cells to the WaTorModel, so that the rules are
   * not processed on them (so their states are not overwritten).
   *
   ...
   *                cell needs to move to an Empty cell or Fish cell (if it is a shark)).
   */
  public void checkState(Cells myCell, int state) {
    Map<Integer, Consumer<Integer>> intMap = Map.of(
        Integer.parseInt(myStatesBundle.getString(SAME)), integers -> keepState(myCell),
  ...
  /**
   * This method is called when the cell cannot move to another state, or if the cell is already
   * empty. It keeps the position and state of the cell as it was before the different rules were
   * run on it.
   *
   ...
   * on current state of the cell, it calls the method to either move the cell to an empty cell
   * (FISH state), or to either an empty cell or a fish cell (SHARK state). Or, if the cell is
   * already empty, it keeps the state of the cell as it already is.
   *
   * @param cell: The current cell being considered in the state-changing process.
   */
  public void moveState(Cells cell) {
    Map<Integer, Consumer<Integer>> intMap = Map.of(
        Integer.parseInt(myStatesBundle.getString(FISH)),
        integers -> moveCells(myDevelopmentStage.findRightState(cell, myNeighbors,
                Integer.parseInt(myStatesBundle.getString(EMPTY))),
            cell),
        Integer.parseInt(myStatesBundle.getString(SHARK)),
        integer -> eatCells(myDevelopmentStage.findRightState(cell, myNeighbors,
                Integer.parseInt(myStatesBundle.getString(FISH))),
            cell),
  ...
  private void eatCells(Cells c, Cells cell) {
    if (c == cell) {
      moveCells(myDevelopmentStage.findRightState(cell, myNeighbors,
          Integer.parseInt(myStatesBundle.getString(EMPTY))), cell);
      return;
    }
    changeNeighborCells(c, cell);
