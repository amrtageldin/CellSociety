 ...
 * It deals with the movement of the cell types A and B depending on the percentage of valid neighbors
 * they have. If the rules class returns that a cell needs to move to a region of different neighbors,
 * this class randomly selects open spots for the cell to move to, and immediately reflects this in
...
public class SchellingSegregationMovement implements CellSocietyMovement{
    ...
    /**
     * This method sets up initial parameters (after being called by SchellingSegregationModel)
     ...
     * @param parameters: the map of all the relevant parameters for the Schelling game as read
     *                  from the .sim file.
     */
     ...
     *             (the cell needs to move to a region with more of its own kind of neighbors)
     */
     ...
     * current state of the cell, it calls the method to either move the cell to a randomly selected empty cell
     * (Cell types A and B are moved). Or, if the cell is already empty, it calls a method to keep the state of
    ...
    /**
     * This method is called when the cell either has enough of its own kind of neighbors,
