 ...
 * the GameOfLife game for CellSociety. It overrides the abstract setNextState()
 * method within the abstract class to set up the next state of a cell as a result
 * of applied GameOfLife-specific rules.
 */
public class GameOfLifeModel extends CellSocietyModel{
     ...
     * @param parameters: All of the relevant parameters for the GameOfLife game. This
     ...
     * CellSocietyModel class. It finds all of the Alive cells within the current
     * cell's list of neighbors. It sends this number of Alive cells within the neighbors
     * into the CellSocietyRules class to determine what the cell's next state should be.
     * At the end, the cell is either made alive if enough neighbor cells are alive,
     * made dead if too many cells are alive or too many neighbor cells are dead, or not
     * changed at all.
