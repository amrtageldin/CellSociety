...
/**
 * This class reads the loaded .sim file and parses it to create a Map of all the relevant parameters
 * for a game. It is called by the controller to initialize the game type so that the correct Model
 * can be loaded to run the intended game. It assumes that the user inserts a valid file, but if
...
public class GameFactory {
     ...
     * into a HashMap with its key being the String before the "=" in the .sim file. It then
     * looks at the first entry to see which game has been loaded up to play. This game type
     * is sent back into the controller, which uses reflection to call the correct Model class
     * for the chosen game type.
    ...
    /**
     * This method returns all of the parameters that were specified in the .sim file that
     * was loaded by the user. This is sent to the controller and used by the view as well as the
     * model to keep track or necessary game parameters.
    ...
    /**
     * This method returns the GameFactory class's instance of ErrorFactory,
