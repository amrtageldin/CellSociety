...
/**
 * This class initializes all of the cells present in a Grid by parsing a CSV file.
 * It has the functionality to either read each cell's state from the CSV file, or generate
 * random states based on valid states for the current game being played. The final Grid object that it
...
public class Grid {
   ...
   * which were read from a CSV via the GridFactory class.
  ...
  /**
   * Given a game type, this method gets all of the possible states within that game so that if a CSV
   * file with unknown states is loaded the Grid will be able to self-populate with the knowledge of
   * which states are relevant to which games.
  ...
  /**
   * This method is called by the GridFactory to initialize each of the cells within the Grid object.
   * It takes in a CSVReader object, and then parses the CSV for each cell so that it can be set up
   * within the Grid. If the cells are "x", then they are randomly generated using the knowledge of
   * valid cell states for the current game. Otherwise, the cell states are read from the CSV file.
  ...
  /**
   * This method sets a cell in the Grid row i and column j to equal another cell. It is
  ...
  /**
   * This method returns the Grid class's instance of ErrorFactory,
   ...
   * show up while loading the .sim file.
